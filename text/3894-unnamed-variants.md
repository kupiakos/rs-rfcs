...
// SAFETY: 5 is a claimed discriminant for `Fruit`.
let fruit: Fruit = unsafe { core::mem::transmute(5u32) };
...
// SAFETY: 7 is a claimed discriminant for `Fruit`
...
enum Fruit {
    ...
    Orange,      // Orange is represented with 1u32.
    Banana = 4,  // Banana is represented with 4u32.
    _ = ..,      // The rest of the discriminants in `u32` are claimed.
}
...
- `start..end` (`core::ops::Range<Int>`) or\
  ...
  - The range must not overlap with discriminants claimed by other unnamed
    variants. Multiple unnamed variants declarations have equal claim to a
    discriminant value.
